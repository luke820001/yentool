...
> 現行規則為 **近 3 年 70.8% / 每筆 +1.95%，更早的資料 69.5% / +2.04%**（556 筆、含費稅）。
...
### 2026-09-23 「實際獲利狀態不佳」：錢的視角（回測登錄簿 K 節）

- [x] 帳本重建：掃描器實際發出的可買訊號 2026-06-25 起 8 筆結束、62.5%、合計 −9.5%；
      研究回測同期 5 筆、−18.8%。**策略本身在這段是虧的，不是實作偏差。**
- [x] 現行規則的損益結構：獲利四成來自 17% 的停利交易；時間出場 20% 平均 −9.9%。
- [x] 五個候選家族用錢計分全數否決（ATR 縮放、晚期砍虧損、固定停利、賣一半）。
- [x] 出場引擎補上續抱腿（`ride_cap`）；`scanner/live_record.py`；手機「帳本實際」。
- [ ] **待擁有者決定：固定資金格數**（10 格年化 18%／回撤 −12%，5 格 25%／−26%，
      3 格 36%／−32%）。先決定能承受的回撤。
- [ ] 方向：ATR ≥ 7.5 的名字用較寬停損（80 筆的桶，等樣本到 200 筆）。
- [x] 實作落差 1（第一天定義）對線上 302 筆基準的乾淨測試 → 純數量損失，已修（`First_Day`，L 節）。
      落差 2（母體）兩窗口相反，不修；F13 的 `gap_tol` 本身維持（那是持有日曆）。
- [x] G 節那條路用錢計分（K.7）：Q5 絕對報酬贏、風險調整打平，交擁有者選目標；加碼半倉是降風險選項。
- [x] `tools/ledger_audit.py`：手機備份對帳工具（可買訊號／停損未執行／提早賣／抱過上限）。
- [ ] **待擁有者提供手機備份 JSON**，跑一次對帳，看漏水在策略還是執行。
