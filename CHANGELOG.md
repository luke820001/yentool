...
## 2026-09-23 （擁有者：「認為目前實際獲利狀態不佳」）

### feat: 手機策略卡多了「帳本實際」，跟回測並排

畫面上每一處都寫 70.8%，擁有者卻在賠錢——兩件事同時為真，而專案裡沒有任何
一個數字能讓人看見。把 `signal_ledger` 裡掃描器**真的發布過**的訊號按買進規則分三桶、
用完整出貨規則重放（`scanner/live_record.py`，每次掃描重算，`meta.live_record`）：

| 2026-06-25 ~ 09-22 | 筆數 | 勝率 | 平均 | 合計 |
|---|---|---|---|---|
| 符合完整買進規則 | 8 結束＋1 進行中 | 62.5% | −1.18% | **−9.5%** |
| 名單上但不可買（未過核心+） | 54 | 59.3% | −4.35% | −235.0% |
| 核心+ 但大盤未順風 | 8 | 62.5% | −1.10% | −8.8% |

研究回測同一段自己也只有 5 筆、−18.8%。兩邊一致：**這三個月策略本身就是虧的**，
不是實作偏差。為什麼「勝率 70% 卻賺不到錢」寫在回測登錄簿 K.1：獲利的四成
來自 17% 碰到停利的交易，三成六的交易只賺 +1.2%，時間出場平均 −9.9%。

### feat: 出場引擎補上「續抱到 20 天」這條腿

`DEFAULT_RULE["ride_cap"] = 20`；`replay_exit` / `simulate_exit` 在第 10 天收盤站上
自己 5 日均價時續抱、最晚 20 天。這條腿之前只活在線上逐日追蹤與研究引擎裡，
產線上沒有東西能把**完整**出貨規則套在一筆結束的交易上。556 筆對研究引擎逐筆比對
0 差異。視窗在續抱途中用完 → 回報「進行中」而不是猜一個數字。
帳本的 5／10／20 日列刻意不續抱（它們是「視窗內的規則」，`ride_cap=None`）。

### fix: 「第一天入榜」改回被驗證的定義（`First_Day`），線上訊號數約翻倍

買進規則的「新訊號」只讀 `Hold_Status == pending`，而它背後的連續段容忍 10 天缺席——
等於要求一檔股票離開名單**超過 10 個交易日**才能再買。研究規則是「前一個交易日不在名單上」。
對「線上 302 筆」基準做的乾淨雙窗口測試（回測登錄簿 L 節）：被丟掉的 178 筆跟留下的一樣好
（近 3 年 68.3% / +2.01 對 69.6% / +1.59），線上的選法落在「隨機刪一半」的區間正中間；
固定資金 10 格年化 8.8% → 16.0%。`holding_tracker` 多一欄 `First_Day`（前一個帳本交易日不在名單上），
`mark_buy_ready` 的新訊號 = `pending` 或 `First_Day`；連續段與持有日曆完全不動。
母體落差（同日成交額前 300）**不修**：兩個窗口結論相反。

### feat: 對帳工具 `tools/ledger_audit.py`

`scanner/live_record.py` 量的是規則，不是擁有者的執行。這個工具讀手機「匯出備份 JSON」，
逐筆回答：買的時候是不是可買訊號（可買／名單上不可買／核心+但大盤未順風／名單外）、
規則從**你自己的成交價**起算會怎麼出場、你實際怎麼出、差多少（`leak_pct`），
並標記 `stop_not_taken`、`sold_early`、`held_past_cap`、`no_signal`。備份檔留在本機，不進 git。

### G 節那條路第一次用錢計分（回測登錄簿 K.7）

停利 +26%、抱 20 天的 Q5（漲到 +10% 才鎖利）10 格年化 25.1% 對現行 18.2%，回撤 −18.2% 對 −12.5%，
風險調整後打平，舊窗口弱——不採用，交擁有者選目標。**加碼半倉在資金層面是降風險選項**：
每格保留 1.5 份之後年化 18.2% → 14.3%、回撤 −12.5% → −9.1%；G.2 的「Q2 每筆 +6.41%」沒算保留的那一半。

### 第三輪回測：改用「錢」計分，五個家族全數否決（回測登錄簿 K 節）

ATR 縮放停損／鎖利（上輪的活線索：+147 點裡 +143 點在 ATR ≥ 7.5 的 80 筆）、
晚期砍虧損 16 格、固定停利 +15/+18/+22/+25/無、漲 10%/15% 賣一半。
新增的門檻：兩窗口每筆平均都升、成對 bootstrap 的平均差區間不跨零、分桶。
固定資金格數表（3 格年化 35.8%/回撤 −32.3% … 20 格 9.0%/−6.4%）交擁有者決定，
它不改規則卻是最大的槓桿。工具：`archive/research/sandbox_money.py`。


## 2026-09-22 （三個擁有者回報的問題）

### feat: 紀錄可以刪除了，持倉頁與績效頁都有

擁有者：「已平倉/封存的，如果當時買進登錄又撤銷，根本沒辦法直接移除該筆數據」、
「應該也要在持倉就可以刪除該筆紀錄」。重現後確認更糟：撤銷唯一那筆買進之後，
部位停在 **status "open"、0 股、0 筆有效成交**，卡片上寫著**「持有中」**、
每天出現在「待處理」提醒你沒估值，而出口只有封存或撤銷——兩個都只是把它搬到
另一張永久表格裡。

現在有「刪除這筆紀錄」，部位列、成交、估值列、凍結的十日成果四個儲存區
**在同一個交易裡一起清掉**，不留孤兒。確認視窗會先講清楚會消失什麼：
幾筆有效成交、多少已實現損益、無法復原，並且告訴你「只是想收起來就用封存」。
沒有任何有效成交且已實現損益為零的紀錄，只會得到一句簡短確認。

這是專案第一個真的刪除動作。報告 5.2 的「事件即使被取消也要保留」仍然成立，
那是「撤銷誤登」的工作；但一筆登錯了又全數撤銷的紀錄，**沒有事件可以保留**。

### fix: 欄位檢測的警告指對了，那是未還原的公司行為

擁有者問「顯示的欄位檢測警告是不是要修正」——四個警告全部集中在同一檔 6949：
月漲跌 −95.3%、壓力距離 2833%、區間緊密度 29.9。查完發現**不是界限太嚴**：

| 日期 | 收盤 | 成交量 |
|---|---|---|
| 2026-08-26 | 1,490.00 | 550 |
| 2026-09-07 | 67.10 | 24,345 |

中間 7 個交易日沒有 K 棒，價格一步 **÷22**，成交量跳了近百倍。台股任何一檔都不可能
單日 ÷22——這是分割或面額變更，不是下跌。結果：MA20 是 511、MA60 是 792，而收盤是 50.8，
App 告訴你這檔一個月跌了 95.3%——**它沒有**。而且 `Integrity_OK` 是 **True**，
這一列被當成可信資料發布，還帶著 `Hold_Status = pending`。

`data_integrity` 的文件字串本來就預見了這種情況，只是把它當軟旗標，理由是
「>10% 的跳動常常是真的行情」。那對 10–20% 成立，對 3 倍不成立。現在：

- 超過 3 倍的階梯算 `split:` **硬錯誤**，`Integrity_OK` 轉為 False，買進閘門會擋。
- **更重要的**：掃描在算任何滾動值之前就把舊基準的 K 棒切掉，所以 MA、
  月漲跌、壓力距離不再跨兩種單位計算。算不出來就是空值，不是錯值。
  （不重新縮放：我們不知道確切比例，把舊價格換算成今天的單位是在發明資料。）

3 倍這個門檻刻意訂得遠高於任何合法的單日變化（含無漲跌幅的首日上市），
所以不會誤擋真的行情。

### 大盤資料日：線上是對的，你看到的是舊快取

擁有者回報手機寫「大盤判定非最新（資料日 2026-09-18）」。線上發布的 payload
是 `as_of_date 2026-09-22`、`is_current true`、順風成立。那個 09-18 的畫面是今天修正**之前**
發布的版本，Service Worker 是 network-first，手機回到前台時會自己重抓。
根因（指數資料比個股晚一天就否決整份名單）已於今日修正並發布。

### 不改：推薦標的的三個實作落差

子代理的結論是「修好就讓訊號翻倍且品質持平或更好」。**我自己量了一次，結果相反。**
把 564 筆依「live 的定義會不會壓掉這個訊號」分開：

| | 近 3 年 | 舊窗口 |
|---|---|---|
| live 會保留 | 70.74% / +1.947% | 69.40% / +1.944% |
| live 會壓掉 | 71.05% / +1.984% | **65.71% / +1.407%** |

近 3 年被壓掉的略好，**舊窗口明顯更差**。依專案自己的「兩個窗口都要成立」
門檻，這不支持改變買進閘門。我的代理指標會**低估**壓抑程度（只數首日訊號），
所以這不是最終結論；但在有一個干淨的兩窗口測試之前，**不動你實際會買什麼**。
