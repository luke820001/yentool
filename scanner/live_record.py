...
def build_live_record(since="2026-06-25", ledger_file=SIGNAL_LEDGER_FILE,
    ...
    out["through"] = str(picks["bar_date"].max())[:10]

    tradable, not_core, regime_closed = [], [], []
    series_cache = {}
    for _, r in cand.iterrows():
        ...
        if core is None or (isinstance(core, float) and core != core):
            ...
            if core is None:
                ...
                out["core_recomputed"] += 1
        else:
            core = bool(int(core))

        fwd = s.iloc[i + 1:i + 1 + DEFAULT_RULE["ride_cap"] + 1]
        if fwd.empty:
            row = {"sid": sid, "sig": sig, "ret": None, "exit": "", "bars": 0}
        else:
            ret, why, bars = _replay(fwd)
            if why == "na":
                continue
            row = {"sid": sid, "sig": sig, "name": r.get("stock_name"),
                   "ret": None if ret is None else round(ret, 2),
                   "exit": why, "bars": bars}
        if core is True and regime.get(sig, False):
            tradable.append(row)
        elif core is True:
            regime_closed.append(row)
        else:
            not_core.append(row)

    out["tradable"] = _bucket(tradable)
    out["tradable"]["trades"] = sorted(tradable, key=lambda x: x["sig"])[-TRADES_KEPT:]
    out["not_core"] = _bucket(not_core)
    out["regime_closed"] = _bucket(regime_closed)
    return out
